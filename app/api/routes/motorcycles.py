...
from app.services.motorcycle_service import register_motorcycle, list_motorcycles_service, get_motorcycle_by_vin_service, update_motorcycle_vin_service
from app.schemas.response import SucessResponse

router = APIRouter(prefix = "/motorcycles", tags = ["motorcycles"]) 

@router.post("/", status_code = status.HTTP_201_CREATED, response_model = SucessResponse[MotorcycleResponse])
...
@router.get("/", status_code = status.HTTP_200_OK, response_model = SucessResponse[list[MotorcycleResponse]])
...
@router.get('/{vin}', status_code = status.HTTP_200_OK, response_model = SucessResponse[MotorcycleResponse])
...
@router.patch('/{vin}', status_code = status.HTTP_200_OK, response_model = SucessResponse[MotorcycleResponse])
